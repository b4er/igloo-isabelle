...
datatype
     msg = Junk             \<comment> \<open>Unparsable junk message\<close>
         | Agent  agent     \<comment> \<open>Agent names\<close>
...
lemma keysFor_insert_Junk [simp]: "keysFor (insert Junk H) = keysFor H"
by (unfold keysFor_def, auto)
...
lemma parts_insert_Junk [simp]:
     "parts (insert Junk H) = insert Junk (parts H)"
apply (rule parts_insert_eq_I)
apply (erule parts.induct, auto)
done
...
lemma analz_insert_Junk [simp]:
     "analz (insert Junk H) = insert Junk (analz H)"
apply (rule analz_insert_eq_I)
apply (erule analz.induct, auto)
done
...
text\<open>Inductive definition of "synth" -- what can be built up from a set of
    ...
    encrypted with known keys.  Agent names are public. Nonces cannot be guessed.\<close>
...
inductive_set
  ...
  | Junk   [intro]:   "Junk  \<in> synth H"
  | Agent  [intro]:   "Agent agt \<in> synth H"           \<comment> \<open>TODO: turn into generic public values\<close>
...
lemmas synth_monotonic = synth_mono [THEN [2] rev_subsetD]


text\<open>NO \<open>Agent_synth\<close>, as any Agent name can be synthesized.
  The same holds for @{term Junk}\<close>
...
lemma Junk_synth [simp]: "Junk \<in> synth H"
by blast
...
lemma Junk_neq_HPair: "Junk ~= Hash[X] Y"
by (unfold HPair_def, simp)
...
lemmas HPair_neqs = Agent_neq_HPair Nonce_neq_HPair Junk_neq_HPair
...
lemmas pushKeys =
  ...
  insert_commute [of "Key K" "Junk" for K]
...
lemmas pushCrypts =
  ...
  insert_commute [of "Crypt X K" "Junk"  for X K]

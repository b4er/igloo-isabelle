...
abbreviation 
  rkeys :: "(role \<times> uid) set \<Rightarrow> key set" where
  "rkeys U \<equiv> sesK`(\<lambda>((x, y), z). #[x,y,z])`(U \<times> (UNIV::nat set))"

lemma rkeys_sesK [simp]: "sesK (#[rol,R,i]) \<in> rkeys U \<longleftrightarrow> (rol, R) \<in> U"
  by (auto simp add: image_def split: prod.splits)


inductive_set 
  fake :: "atom set \<Rightarrow> (role \<times> uid) set \<Rightarrow> chmsg set \<Rightarrow> chmsg set"
  for T and U and H
where 
  fake_Inj:
